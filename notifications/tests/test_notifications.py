...
def notification_template(notification_type):
    ...
    template.html_body = (
        "testi<b>hötömölö</b>ruumis, muuttujan arvo: {{ html_body_var }}!"
    )
    template.save()
...
def test_notification_template_rendering(notification_template):
    ...
    assert (
        rendered["html_body"]
        == "test <b>HTML</b> body, variable value: foo <b>bar</b> baz!"
    )
    ...
    assert (
        rendered["html_body"]
        == "testi<b>hötömölö</b>ruumis, muuttujan arvo: foo <b>bar</b> baz!"
    )
