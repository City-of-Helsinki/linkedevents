...
env = environ.Env(
    ...
    INACTIVE_USER_DELETION_YEARS=(int, 5),
    EXTRA_INSTALLED_APPS=(list, []),
    FIELD_ENCRYPTION_KEYS=(list, []),
    FINANCIAL_ADMIN_EXPIRATION_MONTHS=(int, 6),
    PAID_REGISTRATION_NOTIFICATION_ENABLED=(bool, False),
    PAID_REGISTRATION_NOTIFICATION_PARENT_ORGANIZATIONS=(list, []),
    PAID_REGISTRATION_NOTIFICATION_ORGANIZATIONS=(list, []),
    FULL_TEXT_WEIGHT_OVERRIDES=(dict, {}),
    FULL_TEXT_WORDS_CACHE_SIZE=(int, 40000),
    ...
    # Reducing reservation seat numbers during grace period does not reduce
    # expiration time
    RESERVATION_GRACE_PERIOD_SECONDS=(int, 300),
    ...
    WEB_STORE_REFUND_DEADLINE_DAYS=(int, 7),
...
EVENT_SEARCH_INDEX_REBUILD_BATCH_SIZE = env("EVENT_SEARCH_INDEX_REBUILD_BATCH_SIZE")
EVENT_SEARCH_INDEX_REBUILD_END_TIME_MAX_AGE_MONTHS = env(
...
WEB_STORE_REFUND_DEADLINE_DAYS = env("WEB_STORE_REFUND_DEADLINE_DAYS")
WEB_STORE_WEBHOOK_API_KEY = env("WEB_STORE_WEBHOOK_API_KEY")

# Signup email notifications
PAYMENT_REQUIRED_NOTIFY_THRESHOLD_MINUTES = env(
    "PAYMENT_REQUIRED_NOTIFY_THRESHOLD_MINUTES"
)
...
RESERVATION_GRACE_PERIOD_SECONDS = env("RESERVATION_GRACE_PERIOD_SECONDS")

EVENT_ADMIN_EXPIRATION_MONTHS = env("EVENT_ADMIN_EXPIRATION_MONTHS")
INACTIVE_USER_DELETION_YEARS = env("INACTIVE_USER_DELETION_YEARS")
FINANCIAL_ADMIN_EXPIRATION_MONTHS = env("FINANCIAL_ADMIN_EXPIRATION_MONTHS")

# Feature flag for financial admin notification for paid registrations
PAID_REGISTRATION_NOTIFICATION_ENABLED = env("PAID_REGISTRATION_NOTIFICATION_ENABLED")

# List of parent organization ids whose sub organization financial admins should be
# notified
PAID_REGISTRATION_NOTIFICATION_PARENT_ORGANIZATIONS = env(
    "PAID_REGISTRATION_NOTIFICATION_PARENT_ORGANIZATIONS"
)
# List of organization ids whose financial admins should be notified
PAID_REGISTRATION_NOTIFICATION_ORGANIZATIONS = env(
    "PAID_REGISTRATION_NOTIFICATION_ORGANIZATIONS"
)
...
if CSP_ENABLED:
    ...
    content_security_policy_configuration = {
        "DIRECTIVES": {
            ...
            "form-action": env("CSP_FORM_ACTION"),
            "font-src": [SELF],
