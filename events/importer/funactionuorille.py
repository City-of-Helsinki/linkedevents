...
from dateutil.rrule import (
    rrule, MO, TU, WE, TH, FR, SA, SU, WEEKLY
    )
from django.contrib.postgres.search import TrigramSimilarity
from django.db.models import Value, CharField, Model, DateField
...
from events.models import ImporterTimeLogger, Keyword
...
FUNACTION_URL = 'https://funactionnuorille.fi/wp-json/wp/v2/funaction_class?per_page=100&page='
...
class FunActionImporter(Importer):
    ...
    def setup(self):
        ...
        self.data_source = DataSource.objects.get_or_create(id=self.name,
                                                            name='FunAction',
                                                            api_key='')[0]


        # TODO: Figure out what the organization should be called actually
        self.organization, _ = Organization.objects.get_or_create(origin_id='funactionnuorille',
    ...
    def _create_recurring_event_dates(self, start_date: datetime, end_date: datetime, weekdays: list):
        ...
        return list(rrule(WEEKLY,
                          byweekday=converted_days,
                          dtstart=start_date, 
                          until=end_date))
    ...
    def _parse_slug(self, data:dict):
        ...
        if slug and not isinstance(slug, str): 
            FunException(f"Slug is of type {type(slug)} instead of string in the"
                         f" event {d['origin_id']}.")
            return None
        if slug:
            clean_slug = slug.replace('-',' '
                            ).translate(self.nodigits_trantab
                            ).strip(' ')
            return clean_slug

    def _get_keywords(self, event_data:dict, from_fields:list):
        keywords = set()
        for field in from_fields:
            to_check = event_data[field].split(' ')
    ...
    def _find_keyword_or_split(self, text:str):
        ...
        text = text.strip(' ')
        match = Keyword.objects.annotate(similarity=TrigramSimilarity('name_fi', text)
                                          ).order_by('-similarity'
                                          ).first()
    ...
    def _get_datetime_from_data(self, data: dict, field: str):
        ...
        if value in (None, False, ''):
            return None
        output = parse(value, dayfirst=True).astimezone(pytz.timezone('Europe/Helsinki'))
    ...
    def _map_locations(self, data):
        ...
        src_locations = {HashedLocation(name=i['locations'][0]['name'],
                            address=self._clean_address(i['locations'][0]['address'])
                            ) for i in data if i['locations']}
        ...
        tprek_places = Place.objects.filter(name__in=locations.keys()).values_list('name', 'id', 'street_address')
        found = set()
        for i in tprek_places:
            ...
            if locations[name].fun_address and tprek_address:
                if distance(locations[name].fun_address.replace(' ','').lower(),
                            tprek_address.replace(' ','').lower()) > ACCEPTABLE_ADDRESS_DISTANCE:
                    logger.warning(f'Place {name} has address {locations[name].fun_address} at FunActionNuorille and {tprek_address} at Tprek')
                    locations[name].tprek_address = tprek_address
            else:
                logger.warning(f'Place {name} is missing the address. FunActionNuorille:{locations[name].fun_address}, Tprek: {tprek_address}')
        ...
        for name, address in still_missing.items():
            tprek_place = Place.objects.extra(where=[f"LOWER(REPLACE(street_address,' ',''))='{str(address).replace(' ','').lower()}'"]
                                      ).annotate(similarity=TrigramSimilarity('name', name)
                                      ).order_by('-similarity').values_list('name', 'id')
            if tprek_place:
                found.add(name)
                locations[name].tprek_name, locations[name].tprekId = tprek_place[0]
                if distance(name.replace(' ','').lower(),
                            locations[name].tprek_name.replace(' ','').lower()) <= ACCEPTABLE_ADDRESS_DISTANCE:
                            locations[name].tprek_name = ''
                else:
                    logger.warning(f'Place {name} was mapped to tprek organization {locations[name].tprek_name}')
        ...
        for name, address in still_missing.items():
            ...
            course = [i for i in data if i['locations'] and i['locations'][0]['name']==name][0]
    ...
    def handle_non_tprek_location(self, location_data):
        ...
        tprekId = Place.objects.get(name=name).id
        return tprekId
    ...
    def _log_duplicates(self, places):
        ...
        if multiple_mentions:
            logger.error(f'Following place names match several addresses in FunActionNuorille system: {" ".join(multiple_mentions)}')
    ...
    def _clean_address(self, address):
        ...
        '''
        toponyms = '(aukio|kaari|kaski|katu|kuja|kulma|laituri|linja|metsätie|mäki|penger|piha|polku|porras|portti|puisto|puistokatu|puutarha|raitti|ranta|rinne|saari|silta|suu|tie|tori)'
        expr = re.compile(fr"(([A-ZÄÖ][a-zäåöé]+{toponyms})|(((-){{0,1}}[A-ZÄÖ][a-zåéäö]+\s){{1,2}}{toponyms}))[\s][0-9]{{1,3}}(([\s]{{0,1}}[A-Za-z])|(-[0-9]))?")
        if expr.search(address):
            return expr.search(address).group(0).rstrip() # TODO: implement a moderator-powered way to handle too messy addresses
        else:
            return None
    ...
    def _fetch_paginated_data(self, url):
        ...
        with requests.Session() as s:
            s.headers.update({'User-Agent': ''}) # TODO: without this request returns 403. to be fixed with API providers.
            
            data, code = self._fetch_page(session=s, counter=cnt)
            if code != 200:
                message = f'Could not fetch data, got HTTP code {response.status_code}'
                raise APIBrokenError(message=message)
            while code == 200:
                cnt = cnt + 1
                page, code = self._fetch_page(session=s, counter=cnt)
                if page: data.extend(page)
            return data
