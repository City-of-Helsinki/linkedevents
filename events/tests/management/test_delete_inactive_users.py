...
@freezegun.freeze_time("2024-05-28 03:30:00+03:00")
@pytest.mark.django_db
def test_delete_inactive_users_uses_last_login_when_no_last_api_use():
    now = localtime()
    expiration_years = django_settings.INACTIVE_USER_DELETION_YEARS

    expired_login = now - relativedelta(years=expiration_years + 1)
    active_login = now - relativedelta(years=expiration_years, days=-1)

    # last_api_use is None — fall back to last_login
    expired_user = UserFactory(last_api_use=None, last_login=expired_login)
    active_user = UserFactory(last_api_use=None, last_login=active_login)
...
def test_delete_inactive_users_deletes_user_with_soft_deleted_signup_as_contact_person():
    ...
    # Soft-delete the contact person and the signup
    signup.soft_delete()

    call_command("delete_inactive_users")

    # User must be deleted — only soft-deleted signups remain
...
def test_delete_inactive_users_deletes_user_with_soft_deleted_signup_group():
    ...
    # User must be deleted — the group and its signups are soft-deleted
    assert not User.objects.filter(pk=user.pk).exists()
