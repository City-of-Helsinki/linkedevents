...
load = {'name': 'Launchpad McQuack', 'subject': 'adventure', 'body': 'Huey, Dewey, and Louie are waiting for you Sir!',
        'email': 'test@test.com'}
...
def test__signed_in_user_submits(api_client, user):
    ...
    assert mail.outbox[0].subject == f"[LinkedEvents] {load['subject']} reported by {load['name']}"
