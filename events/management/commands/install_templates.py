...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        city_template_dir = os.path.join(
            settings.BASE_DIR, options["city_directory"][0], "templates/rest_framework/"
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Successfully installed the template from "
                + options["city_directory"][0]
            )
        )
