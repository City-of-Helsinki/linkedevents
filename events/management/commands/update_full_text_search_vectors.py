...
from events.models import Event, XEventFullText
from events.utils import get_field_attr, get_word_bases
...
def update_full_text_search_vectors():
    """
    Updates the full text search vectors for the events.
    """
    num_populated = 0
    ...
    for event in qs.iterator(chunk_size=10000):
        ...
        # create or update EventFullText object
        XEventFullText.objects.update_or_create(
            event=event,
            defaults={
    ...
    # populate search_vector_fi
    eqs = Event.objects.filter(x_full_text=OuterRef("pk"))
    XEventFullText.objects.all().annotate(
        event_name=Subquery(eqs.values("name_fi")[:1]),
        event_description=Subquery(eqs.values("description_fi")[:1]),
