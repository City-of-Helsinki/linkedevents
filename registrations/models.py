...
class Registration(CreatedModifiedBaseModel):
    ...
    def create_or_update_web_store_product_mapping_and_accounting(self):
        ...
        resp_json = create_web_store_product_mapping(
            self.to_web_store_product_mapping_data()
        )

        product_id = resp_json["productMapping"]["productId"]

        create_web_store_product_accounting(
            product_id, self.to_web_store_product_accounting_data()
        )
